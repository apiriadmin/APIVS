...
typedef union argValue
{
	...
	FIO_NOTIFY_INFO	fioInfo;			// FIO_NOTIFY_INFO
...
extern	int16_t	argVerifyInitCompat(uint16_t	ln,
									ARG_P		*pTo,
									const char	*pMember,
									ARG_P		*pFrom);
...
extern	int16_t	argDefineVar(uint16_t 	ln,
							 const char *pName,
							 VAR_TYPE 	vt,
							 uint16_t 	size);
...
extern	int16_t argDereferenceVar(uint16_t		ln,
								  const char	*pName,
								  ARG_P			**ppVar);

/** \brief Dereference a numeric */
extern	int16_t argDereferenceNumeric(uint16_t		ln,
									  const char	*pName,
									  ARG_P			**ppVar);

/** \brief Dereference a string */
extern	int16_t argDereferenceString(uint16_t		ln,
									 const char		*pName,
									 ARG_P			**ppVar);

/** \brief Dereference a macro */
extern	int16_t argDereferenceMacro(uint16_t		ln,
									const char		*pName,
									ARG_P			**ppVar);

/** \brief Dereference a Named Constant */
extern	int16_t argDereferenceNC(uint16_t		ln,
								 const char		*pName,
								 ARG_P			**ppVar);

/** \brief Dereference an argument */
extern	int16_t argDereferenceArg(uint16_t ln,
								  const char *pName,
								  ARG_P	**ppVar);
...
extern	int16_t argSetVar(uint16_t	ln,
						  OP_TYPE	operation,
						  ARG_P		*pValue,
						  ARG_P		*pFile,
						  ARG_P		*pVar,
...
extern	int16_t	argCastInt(uint16_t	ln,
						   ARG_P	*pArg,
						   int		*pInt);
...
extern	int16_t	argCompareType(uint16_t ln,
							   ARG_P 	*pArg,
							   char		*pMember,
							   boolean	*comp);
...
extern	int16_t	argSet(uint16_t			ln,
					   const char		*pPath,
					   const char		*pFile,
					   unsigned char	**ppBuf,
					   unsigned int				*pSize);
